...
class _class extends _cliEngineCommand2.default {
  ...
  async run() {
    let plugins = await new _plugins2.default(this.out).list();
    ...
    for (let plugin of plugins) {
      ...
      if (plugin.type !== 'user') output += this.out.color.gray(` (${plugin.type})`);else if (plugin.tag !== 'latest') output += this.out.color.gray(` (${String(plugin.tag)})`);
      this.out.log(output);
    }
  }
}
